...
from src.risk_model import fit_risk_stack, risk_features
...
class RiskModelTests(unittest.TestCase):
    def test_stack_features_include_oof_feature_slots(self) -> None:
        ...
        values = snapshots.target_final_delay_hours
        matrix = risk_features(
            snapshots, values * 0, values.gt(12).astype(float), values
        )
    ...
    def test_platt_calibrator_receives_complete_oof_raw_probabilities(self) -> None:
        ...
        train_rows = snapshots.loc[snapshots.shipment_id.isin(train_ids)]
        validation_rows = snapshots.loc[snapshots.shipment_id.isin(validation_ids)]
        train_shipments = shipments.loc[shipments.shipment_id.isin(train_ids)]
        _, validation_eta = fit_eta(train_rows, train_shipments, validation_rows)
        models, predicted = fit_risk_stack(
